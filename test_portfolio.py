...
class TestValuationDate:
    def test_default_valuation_date_is_current_date(self, monkeypatch):
        import datetime as dt
        ...
        assert main._resolve_as_of(None) == dt.date(2026, 6, 30)

    def test_explicit_valuation_date_overrides_current_date(self):
        import datetime as dt

        assert main._resolve_as_of("2026-06-21") == dt.date(2026, 6, 21)

    def test_invalid_explicit_valuation_date_is_rejected(self):
        with pytest.raises(ValueError, match="YYYY-MM-DD"):
            main._resolve_as_of("21/06/2026")
...
class TestSummaryJson:
    def test_summary_json_includes_generating_skill_metadata(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
...
class TestHtmlReport:
    ...
    def test_html_includes_generating_skill_metadata(self):
        holdings = pd.DataFrame(
            {"ticker": ["A"], "name": ["Alpha"], "shares": [10],
             "avg_price": [100.0], "cost_basis": [1000.0],
             "allocation_pct": [100.0], "last_price": [110.0],
        ...
        assert "Generated by xtb-portfolio-review v1.0.10" in html
