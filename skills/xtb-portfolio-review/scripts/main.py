...
def load_skill_metadata() -> dict[str, str]:
    """Read this copied skill's name and version from SKILL.md front matter."""
    metadata = {"skill": DEFAULT_SKILL_NAME, "version": UNKNOWN_SKILL_VERSION}
    ...
    for raw_line in front_matter.splitlines():
        ...
        if normalized == "name":
            metadata["skill"] = value.strip().strip("'\"") or DEFAULT_SKILL_NAME
        elif normalized == "version":
            metadata["version"] = value.strip().strip("'\"") or UNKNOWN_SKILL_VERSION
    return metadata
